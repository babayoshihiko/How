...
```{r echo = FALSE}
# Data: prevalence (proportion) and sample size
prev <- c(0.45, 0.50, 0.55)
n    <- c(100, 200, 300)

# Convert prevalence to number of events
events <- prev * n

# Create effect sizes (logit transformed proportions)
...
# Forest plot
forest(res, 
       transf = transf.ilogit, # back-transform logits to proportions
       refline = 0.5,          # reference line at 50%
       xlab = "Prevalence",
       slab = paste("Study", 1:3))
```

このとき、Cochran Q は `r res$QE` (p = `r res$QEp`) で、I$^2$ = `r res$I2` です。

```{r echo = FALSE}
# Data: prevalence (proportion) and sample size
prev <- c(0.10, 0.50, 0.90)
n    <- c(100, 200, 300)
